...
public class Automobile {
	...
	public void refuel(double dollarAmount) {
		...
		// If the entire dollarAmount worth of fuel cannot fit into the tank, stop
		// fueling
		...
		// display �Please pay XX.XX dollars for X.XX gallons.� to add to full gas
		// tank?????
		if (canBuyGallons < emptyFuelLevel) {

			setFuelLevel(getFuelLevel() + canBuyGallons);
			System.out.println("Please pay $" + GAS_PRICE * canBuyGallons + " for " + canBuyGallons);
		}
		if (canBuyGallons >= emptyFuelLevel) {
			double payForGas = emptyFuelLevel * GAS_PRICE;
			double change = dollarAmount - payForGas;
			setFuelLevel(FUEL_TANK_SIZE);
			System.out.println("Please pay " + dollarAmount + " dollars for " + emptyFuelLevel
					+ " gallons. Your change is " + change + " dollars");
		}
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("Fuel level" + a.getFuelLevel());
		a.refuel(60);
		System.out.println("Fuel level" + a.getFuelLevel());
		a.drive(350);
		System.out.println("Fuel level" + a.getFuelLevel());
